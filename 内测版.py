...
pos_y1 = pos_y2 = pos_y3 = pos_y4 = pos_y5 = pos_y6 = 490  # 前剧情文本y坐标(在界面下方)
...
def draw_subtitles(text, tick, pos_x, pos_y, factor, time1=0, time2=0):
    ## global speed_text
    if time1 <= tick <= time2:
        position = pos_x, pos_y
        pos_y += speed_text
        if factor <= 1:
            factor += speed_factor1

        win.blit(drawText(text, blend_color(blend_factor=factor)), position)

## list_text1 = [text1, pos_x1, pos_y1]
## for i in range(len(list_text_a))
    ## position_a[i] = pos_x_a[i], pos_y_a[i]
    ## pos_y_a[i] += speed_text
    ## win.blit(drawText(text_a[i], blend_color(blend_factor = factor_a[i])), position_a[i])
    ## iif factor_a[i] <= 1: factor_a[i] += speed_factor[i]
...
while True:
    ...
    if game_over:  # 在游戏开始时(封面)
        ...
        win.blit(drawText("测试版", white, 20), (975, 470))
        ticks_h = tisks_ab

    elif game_pause:  # 在游戏暂停时
        win.fill(black)
        win.blit(drawText("<点击以继续>", dark_red, 35), (430, 200))
        ...
        draw_subtitles(text1, ticks, pos_x1, pos_y1, factor1, time2=6000)  # 文本1
        ...
        if 13000 <= ticks and choose_1 == 0:  # 第一次选择
